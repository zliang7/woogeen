...
function getToken(addr, user, room_id) {
    if (addr.indexOf("http") < 0) {
        addr = "http://" + addr + "/createToken";
    }
    var url = require("url").parse(addr);

    var body = '{"role":"presenter","username":"' + user + '"'; // jshint ignore:line
    if (room_id) {
        body += ',"room":"' + room_id + '"'; // jshint ignore:line
    }
    body += "}";
    log.I("getToken body:" + body);
...
ConferenceClient.prototype = {
    ...
    getConnectionStats: function(stream, regionId) { // jshint ignore:line
...
var woogeen = {
    ...
    LocalCustomStream: bindings.LocalCustomStream,
